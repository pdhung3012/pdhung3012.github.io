...
public class InserirFaturamentoCronogramaAction extends GcomAction {
     ...
     * pelo usuario faz alguma valida��es e envia os objetos(FaturamentoGrupoCronogramaMensal, FaturamentoAtividadeCronogra)
     * para inser��o, onde ser� feito a valida��o, parte de negocio, e inserido o objeto.
     *
     * @param actionMapping
    ...
    public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse) {
		...
		while (iterateFaturamentoAtividades.hasNext()) {
			...
			FaturamentoCronogramaAtividadeHelp help = (FaturamentoCronogramaAtividadeHelp) iterateFaturamentoAtividades.next();

			faturamentoAtividadeCronograma.setFaturamentoAtividade(help.getFaturamentoAtividade());
			String dataPrevista = (String) httpServletRequest.getParameter("n" + faturamentoAtividadeCronograma.getFaturamentoAtividade().getId().toString());
				...
				if (idsAtividadesComandadas != null && !idsAtividadesComandadas.equals("")) {
					for (int i = 0; i < idsAtividadesComandadas.length(); i++) {
						...
						if (Util.validarStringNumerica(idAtividadeAux)) {
							if (unknownMethod() == faturamentoAtividadeCronograma.getFaturamentoAtividade().getId().intValue()) {
		...
		fachada.validarFaturamentoCronogramaAtividadeMaiorQueMesAnoCronograma((new Integer(mesAno)).intValue(),	colecaoTestePredecessoraVazia);
		fachada.validarFaturamentoCronograma(colecaoTestePredecessoraVazia);

		fachada.inserirFaturamentoGrupoCronogramaMensal(faturamentoGrupoCronogramaMensal, faturamentoAtividadeCronogramas, usuarioLogado, new Integer(mesAno));

		// pega o grupo de faturamento para mostrar a descricao na tela de
		// sucesso
		...
		// pega o id do faturamento grupo cronograma mensal para o caso de
		// chamar direto o atualizar
		...
		filtroFaturamentoGrupoCronogramaMensal.adicionarParametro(
				new ParametroSimples(FiltroFaturamentoGrupoCronogramaMensal.ANO_MES_REFERENCIA, mesAno));
		...
		montarPaginaSucesso(httpServletRequest,
				"Cronograma de Faturamento do grupo " + faturamentoGrupoDescricao.getId() + " referente ao m�s/ano: "
						+ faturamentoGrupoCronogramaMensal.getMesAno() + " inserido com sucesso.",
				"Inserir outro Cronograma de Faturamento.", "exibirInserirFaturamentoCronogramaAction.do?menu=sim",
				"exibirAtualizarFaturamentoCronogramaAction.do?idRegistroAtualizacao="
						+ faturamentoGrupoCronogramaMensalComId.getId(),
